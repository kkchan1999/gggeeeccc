...
void* routine(void* arg)
{
    ...
    while (1) {
        ...
        (p->task)(pool->man_list, &(p->info)); //函数调用
...
bool init_pool(thread_pool* pool, unsigned int threads_number)
{
    ...
    pool->man_list = malloc(sizeof(struct running_man)); //跑腿人员链表
...
bool add_task(thread_pool* pool,
    void (*task)(struct running_man*, struct user_info*), void* arg)
{
    ...
    new_task->info = *(struct user_info*)arg;
...
int add_thread(thread_pool* pool, struct running_man* runman)
{
    struct running_man* new_node;

    new_node = malloc(sizeof(struct running_man));

    *new_node = *runman;

    new_node->next = NULL;

    pthread_create(&(new_node->tid), NULL, routine, pool);

    struct running_man* tmp = pool->man_list; //将线程池中任务链表的头节点登记到tmp
    while (tmp->next != NULL) //将tmp指向最后的节点的位置
        tmp = tmp->next;

    tmp->next = new_node; //将新建的任务节点插入到链表中
