...
bool init_pool(thread_pool* pool, unsigned int threads_number)
{
    ...
    pool->info_list = malloc(sizeof(info_node_t)); //申请新的链表头
    pool->tids = malloc(sizeof(pthread_t) * MAX_ACTIVE_THREADS); //申请最大线程数量的ID内存

    if (pool->task_list == NULL || pool->tids == NULL || pool->info_list == NULL) //判断两个申请的内存是否成功
    {
    ...
    pool->info_list->next = NULL; //初始化新的链表头，将下一个节点指向NULL
...
int add_thread(thread_pool* pool, unsigned additional_threads)
{
    ...
    unsigned total_threads = pool->active_threads + additional_threads; //将总数记录在这个变量中
...
{
    info_node_t* new_node;

    new_node = malloc(sizeof(info_node_t));
    if (new_node == NULL) {
        perror("request info_node memory error");
        return false;
    }
    ...
    info_node_t* tmp = pool->info_list; //将线程池中任务链表的头节点登记到tmp
    while (tmp->next != NULL) //将tmp指向最后的节点的位置
        tmp = tmp->next;
